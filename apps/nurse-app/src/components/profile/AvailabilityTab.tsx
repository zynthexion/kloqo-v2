...
import { Clock, Calendar, Edit, Save, Trash2, PlusCircle, AlertTriangle, Loader2 } from "lucide-react";
...
export function AvailabilityTab({ doctor, onUpdate, isPending, mode = 'full' }: AvailabilityTabProps) {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <Card className="border-none shadow-2xl shadow-black/5 rounded-[2rem] bg-white overflow-hidden">
        <CardHeader className="p-8 border-b border-slate-50 bg-slate-50/50 flex flex-row items-center justify-between space-y-0">
          ...
          {mode === 'full' && (
            !isEditingSettings ? (
              <Button onClick={() => setIsEditingSettings(true)} variant="ghost" className="rounded-xl font-black uppercase text-[10px] tracking-widest text-slate-400 hover:text-slate-800 hover:bg-slate-100">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => setIsEditingSettings(false)} className="rounded-xl bg-slate-800 font-black uppercase text-[10px] tracking-widest text-white px-6">Apply</Button>
              </div>
            )
          )}
        </CardHeader>
      ...
      <Card className="border-none shadow-2xl shadow-black/5 rounded-[2rem] bg-white overflow-hidden">
        <CardHeader className="p-8 border-b border-slate-50 bg-slate-50/50 flex flex-row items-center justify-between space-y-0 text-slate-400">
          ...
          {mode === 'full' && (
            !isEditing ? (
              <Button onClick={() => setIsEditing(true)} variant="ghost" className="rounded-xl font-black uppercase text-[10px] tracking-widest text-slate-400 hover:text-slate-800 hover:bg-slate-100">
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={form.handleSubmit(handleSave)} disabled={isPending} className="rounded-xl bg-slate-800 font-black uppercase text-[10px] tracking-widest text-white px-6 shadow-lg shadow-black/20">
           ...
           {isEditing ? (
             <Form {...form}>
               <div className="space-y-6">
                 ...
                 <Button type="button" variant="outline" className="w-full h-16 rounded-[2rem] border-2 border-dashed border-slate-200 text-slate-400 font-black uppercase text-xs tracking-widest hover:border-slate-800 hover:text-slate-800 transition-all" onClick={() => append({ day: "Monday", timeSlots: [{ from: "09:00", to: "17:00" }] })}>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {doctor.availabilitySlots?.map(slot => (
                 <div key={slot.day} className="p-6 rounded-[2rem] bg-white border-2 border-slate-50 shadow-sm relative overflow-hidden group">
                   <div className="absolute top-0 right-0 w-2 h-full bg-slate-500/5 group-hover:bg-slate-800/10 transition-colors" />
                   ...
                   <div className="space-y-2">
                     {slot.timeSlots.map((ts, i) => (
                       <div key={i} className="flex items-center gap-2 text-xs font-black text-slate-500 bg-slate-50 py-2 px-3 rounded-xl border border-slate-100/50">
                         <Clock className="h-3 w-3 text-slate-800/60" strokeWidth={3} />
