...
export default function PatientRegistrationForm() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        if (!clinicId) {
            toast({ 
                variant: 'destructive', 
                title: 'No Clinic Selection', 
                description: 'Please search for a clinic and book an appointment to register.' 
            });
            return;
        }
        ...
        try {
            ...
            const response = await apiRequest('/patients/manage', {
                method: 'POST',
                body: JSON.stringify({
                    name: values.name,
                    age: values.age,
            ...
            // If we have a user logged in, their patientId is NOT automatically linked by /patients/manage (which just creates the patient doc)
            // But the next checkAuth() call will trigger the backend sync logic if it's there.
            // Or better, let's trigger it ourselves in AuthContext (see next edit).
        ...
        <div className="w-full h-full flex flex-col items-center justify-center bg-muted/20 p-4">
            <Card className="w-full max-w-sm rounded-2xl border bg-card shadow-lg">
                <CardHeader className="text-center">
                    ...
                    <CardDescription>
                        Complete your profile to manage appointments and tokens.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {isSubmitted ? (
                        <div className="flex flex-col items-center justify-center text-center gap-4 py-8">
                            ...
                            <Button onClick={() => router.push('/live-token')} className="w-full mt-4">
                                Go to Dashboard
                            </Button>
                        </div>
                    ) : (
