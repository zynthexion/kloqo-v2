...
export function PrescriptionCanvas({ 
  ...
  const drawMetadata = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // Inner spacing for patient details
    const patientFields = [
    ...
    ctx.font = '800 12px "Inter", "PT Sans", sans-serif';
    ctx.fillStyle = '#e2e8f0';
    ctx.textAlign = 'right';
    ctx.fillText('ADVICE / INSTRUCTIONS', width - margin, pbTop + 140);
  ...
  }, [strokes, currentStroke, doctor, clinic, patient, appointment, drawMetadata, getSvgPathFromStroke]);
  ...
  const handleSave = (type: 'complete' | 'print' = 'complete') => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    if (strokes.length === 0) {
    ...
    canvas.toBlob((blob) => {
      if (!blob) return;
      if (type === 'complete') onComplete(blob);
      else if (onPrint) onPrint(blob);
    }, 'image/jpeg', 0.85);
  };

  return (
    <div className="flex flex-col h-full w-full bg-white select-none relative group overflow-hidden">
      <canvas
        ref={canvasRef}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center bg-white/90 backdrop-blur-xl border border-slate-200 shadow-2xl rounded-2xl py-2 px-3 gap-2 transition-all duration-300">
        <button
          onClick={clearCanvas}
          className="flex flex-col items-center justify-center w-16 h-14 rounded-xl hover:bg-red-50 text-slate-400 hover:text-red-500 transition-all font-pt-sans"
        >
        ...
        <button
          onClick={onSkip}
          className="flex flex-col items-center justify-center w-16 h-14 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-slate-900 transition-all font-pt-sans"
        >
          <Pause className="h-5 w-5 mb-1" />
          <span className="text-[10px] font-bold uppercase tracking-tighter">Skip</span>
        </button>
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-3 h-14 px-6 rounded-xl bg-slate-900 text-white shadow-lg shadow-slate-900/20 hover:shadow-slate-900/30 active:scale-95 transition-all text-sm font-bold font-pt-sans",
          ...
          {isSubmitting ? (
            ...
            <>
              ...
              <div className="flex flex-col items-start leading-none">
                <span className="uppercase tracking-widest text-[10px] opacity-70">Submit</span>
